...
Использование метрики Хаусдорфа сопряжено с некоторыми трудностями, так как при стремлении длины промежутка времени к нулю множества достижимости стягиваются в точки, и расстояние Хаусдорфа будет стремиться к нулю независимо от формы множеств.
...
%Четвертый подраздел посвящён оценке погрешностей в значении функционала при использовании линейной обратной связи. 
...
\end{gather*}
Затем рассмотрим эквивалентное ему неравенство
...
\begin{gather}\label{s2:constrY}
 \lVert u(\cdot)\rVert^2_{\mathbb{L}_2} = \left(u(\cdot),u(\cdot) \right)_{\mathbb{L}_2} = \int\limits_{t_0}^{t_0 + \overline{\varepsilon}} u^{\top}(\tau) u(\tau) \ d\tau  \leqslant \mu^2.
\end{gather}
...
\begin{definition}
 ...
 \begin{gather*}
 \overline{W}(t_0 + \varepsilon) = \int_{t_0}^{t_0+\varepsilon} C X(t_0+\varepsilon,t)B(t)B^{\top}(t)X^{\top}(t_0+\varepsilon,t) C^\top \, dt=CW(t_0 + \varepsilon)C^\top,
 \end{gather*}
...
Система \eqref{s2:linear_system} управляема (вполне управляема) на $ [t_0, t_0 + \varepsilon] $ по выходу $y=Cs$ тогда и только тогда,
...
Множество $ \overline{G}_{1,2}(\varepsilon) $, полученное в численном эксперименте, показано на Рисунке~\ref{fig:u=0_x1-x2}. 
...
Результат численного моделирования приведён на Рисунке~\ref{fig:u=0_x1-x3}.
...
Результаты численного моделирования, приведённые на Рисунке~\ref{fig:u=1_x1-x2}, показывают невыпуклость проекции. 
...
Соответствующий результат численного построения проекции множества достижимости показан на Рисунке~\ref{fig:u=1_x1-x3}.
...
Выпуклость этой проекции проиллюстрирована на Рисунке~\ref{fig:u=1_x2-x3}. 
Как и на Рисунке~\ref{fig:u=0_x2-x3}, здесь пунктирной линией показана точная граница множества достижимости линеаризованной системы.
 \end{enumerate}
\end{document}