...
\begin{document}
\clearpage
\setcounter{section}{0}% Сбрасываем счётчик в 0 (чтобы первое приложение было A)
...
На интервале времени $ t_0 \leqslant t \leqslant {T} $ рассмотрим нелинейную систему:
...
Ключевым вопросом является конструирование множества управлений $U$.
 ...
 В случае же интегральных квадратичных ограничений, систему можно перевести в любую точку множества достижимости (в том числе в граничную), используя только непрерывные управления.
 Поэтому достаточно перебирать непрерывные функции. 
 Рассматриваемый метод предполагает заполнение этого множества управлениями вида 
 \begin{gather}\label{a1:control}
 	u_i(t) = C_i p (t),
 \end{gather}
 где $ i = 1, \dots, N$, $p(t) = \big(p_{0}(t),p_{1}(t),\dots,p_{k}(t)\big)$, $p: [t_0, {T}] \rightarrow \mathbb{R}^{k+1} $ --- вектор-функция, состоящая из ортонормированных скалярных функций в пространстве $\mathbb{L}_2[t_0, {T}]$, а \linebreak $C_i \in \mathbb{R}^{r \times (k+1)}$~---~матрица коэффициентов. 
 ...
 Описанный алгоритм построения множества достижимости системы \eqref{a1:common_nonlinear} с ограничениями \eqref{a1:constraints} легко реализовать, что видно из псевдокода Алгоритма \ref{ap:method}.
 \RestyleAlgo{ruled}
 \begin{algorithm}[hbt!]
 ...
 Итерации Алгоритма \ref{ap:method} независимы друг от друга, что позволяет вычислять концы траекторий $x_i$ параллельно. 
 ...
 \begin{figure}[ht!] 
 	...
 	\begin{minipage}[b]{.3\linewidth} 
 		...
 		\subcaption{$ N = 5\cdot10^4 $, $ k = 15 $, $ t = 4\pi $ } 
 ...
 Первый метод состоит в генерации $r (k + 1)$ равномерно распределённых скаляров $ \overline{c}_{i, j}$, \mbox{$ j = 0, \dots, r (k + 1)-1$}, в диапазоне $[-1;1] $, составлении из них матрицы $\overline{C_i} \in \mathbb{R}^{r \times (k+1)}$ и нормировании её:
 \begin{gather*}
 		C_i = \frac{\mu}{\|\overline{C_i}\|}\overline{C_i}.
 \end{gather*}
 ...
 \begin{figure}[ht]
 	...
 	\caption{Распределение коэффициентов $C_i$ и $\widetilde{C}$.}
 ...
 Тогда матрица коэффициентов может быть восстановлена \linebreak \mbox{$\widetilde{C} = M^{\top} (M M^{\top})^{-1} l + z$}, где $z \in \operatorname{null}(M)$. 
 Здесь $\operatorname{null}(M)$~---~нулевое подпространство матрицы $M$, $\operatorname{null}(M) = \{x \in \mathbb{R^n}, Mx=0\}$
 
 На Рисунке \ref{fig:ap:three_coefficients_distribution} в первых двух колонках приведены гистограммы распределений коэффициентов $C_i$, полученных при помощи описанных двух методов, а в третьей колонке --- коэффициентов $\widetilde{C}$, восстановленных из действительно равномерно распределённых по множеству достижимости точек. 
 ...
 \begin{gather*}
 	...
 	\mbox{при условии} 	\int\limits_0^1 u^2dt \leqslant 1.
 \end{gather*}
 ...
 \end{gather}
 	 ...
 	 \textbf{Пример 2}. На Рисунке \ref{fig:LinearDubins} показана проекция множества достижимости системы 
 \begin{gather}\label{ap:Linear+Dubins}
 	...
 	\end{pmatrix} u.
 \end{gather}
 	на плоскость двух первых координат для момента времени $t = 1$, различных значений $\varepsilon$, при нулевых начальных условиях $x_1(0) = x_2(0) = x_3(0) = 0 $ и таких же ограничениях на управление, как в предыдущем примере, но на интервале времени $0 \leqslant t \leqslant 1$.
