...
\end{gather*}
в начало координат в момент $T$ и обеспечивал при этом минимальное значение $I(T,u)$. 
...
\end{lemma}
\doc 
 Следует из факта, что наибольшее и наименьшее собственные числа матрицы $C$ равны $1/{\lambda_{\min}(D)}$ и $1/{\lambda_{\max}(D)}$, соответственно. \hfill $ \square $
...
Далее будем считать систему \eqref{s22:linear_} полностью управляемой, поэтому $\eta(\tau) \geqslant \nu(\tau) > 0$ при $\tau \geqslant 0$.

Если выполнено Предположение \ref{s22:asm1}, то $\Phi(T)$ непрерывная и возрастающая функция на $ [0,\overline{T}]$ и $\Phi(T) < \infty $.
...
%\begin{enumerate}
 ...
 %\item The smallest eigenvalue of the Gramian of system \eqref{s22:linear_} satisfy the inequality \eqref{s22:asymp} for all $\tau \in (0, %\tau_0] $, where $\tau_0$ is a positive number.
%\end{enumerate} Then $V_{T}(t,z(t)) = z^{\top}(t)Q_T(t)z(t) $ tends to $ 0$ as $t $ tends to $T$.} 
\doc 
Заметим, что функция $ u(\xi) = -B^{\top} Q_{T}(\xi) z(\xi)$  непрерывна на интервале $0 \leqslant \xi <T$. 

Также заметим, что функция $V_{T}(t,z(t))$ может не быть ограниченной только в окрестности точки $t = T$. 
% However, taking into account that the fulfillment of the inequality \eqref{s22:asymp} implies the fulfillment of the assumption, one can see that it follows from $z(t) \to 0 $ that the conditions of lemma \ref{s22:lem:vest} will be met for $t$ from the neighborhood of $T$.
Однако, учитывая, что выполнение неравенства \eqref{s22:asymp} предполагает выполнение условия теоремы, можно увидеть, что из $z(t)\to 0 $ следует, что условия леммы~\ref{s22:lem:vest} будут выполняться для $t$ из окрестности $T$.
