...
Каждому управлению $ u(\cdot) \in \mathbb{L}_2[t_0, \overline{T}] $ соответствует единственное абсолютно непрерывное решение (траектория) $ x(t)=x(t,u(\cdot)) $ системы \eqref{sec1:common_nonlinear}, удовлетворяющее условию $ x(t_0, u(\cdot)) = x_0$ и определённое на интервале $ [t_0, t_0 + \Delta] $, $\Delta > 0$, $ t_0 + \Delta < \overline{T}$.
...
\begin{assumption}\label{sec1:as:right_hand_side_conditions_global}
    Существует такое $\overline{\mu} > 0 $, что все решения (траектории) $ x(t, u(\cdot)) $ системы \eqref{sec1:common_nonlinear}, отвечающие управлениям $u(\cdot) \in B_{\mathbb{L}_2[t_0, \overline{T}]}(0,\overline{\mu})$,  определены на интервале $ [t_0,\overline{T}] $ и лежат в некотором выпуклом компакте $D \subset \Omega \subset \mathbb{R}^n$. 
\end{assumption}
    
Заметим, что из Предположения  \ref{sec1:as:right_hand_side_conditions_global} следует, что $x_0 \in D$, будем считать, что $x_0 \in \operatorname{int} D$, где $\operatorname{int} D $~--- множество внутренних точек $D$.

В частности, Предположение \ref{sec1:as:right_hand_side_conditions_global} выполняется, если  функции $f_1$ и $f_2$ удовлетворяют в $ [t_0, \overline{T}] \times \mathbb{R}^n$ условиям:
...
В \cite[Теорема 5]{Fillipov2} доказано, что из условия \eqref{sec1:right_hand_side_condition} следует продолжимость траекторий. 
...
Известны также другие  оценки на правую часть системы, при наличии которых справедливо Предположение  \ref{sec1:as:right_hand_side_conditions_global} (см., например, \cite{Fillipov2, Guseinov2010}).
...
\begin{lemma}\label{sec1:lem:lip_of_solutions_global}
    Пусть выполнено Предположение \ref{sec1:as:right_hand_side_conditions_global}.
...
Так как для $0 \leqslant \xi \leqslant 1 $ и  $\tau \in [t_0,\overline{T}]$ выполняется включение $x_2(\tau) + \xi \big(x_2(\tau) - x_1(\tau)\big) \in D$, то из Предположения \ref{sec1:as:right_hand_side_conditions_global}, учитывая непрерывность производных $\frac{\partial f_1}{\partial x} $ и $\frac{\partial f_2}{\partial x}$, а также компактность множества $D$, мы имеем, что 
...
Далее будем рассматривать только такие управления $u(\cdot) \in \mathbb{L}_2[t_0, \overline{T}]$, для которых справедливо
\begin{gather}\label{sec1:zero_tail}
	u(t) = 0, \qquad \mbox{при всех} \qquad T \leqslant t  \leqslant \overline{T}.
\end{gather}

Очевидно, что на интервале $[t_0, T]$ такие управления лежат в пространстве $\mathbb{L}_2[t_0, T]$. 
Более того, если $u(\cdot) \in B_{\mathbb{L}_2[t_0, \overline{T}]}(0,\mu) $ и выполняется \eqref{sec1:zero_tail}, то $u(\cdot) \in B_{\mathbb{L}_2[t_0, T]}(0,\mu) $. 
...
Дополнительно заметим, что свойства системы \eqref{sec1:common_nonlinear}, такие как непрерывность функций $f_1$, $f_2$ и их производных, а также единственность решений и ограниченность множества решений (Предположение \ref{sec1:as:right_hand_side_conditions_global}), которые предполагались справедливыми на интервале $[t_0, \overline{T}]$, будут справедливы и на интервале $[t_0, T]$.
...
Решение системы \eqref{sec1:linearized_system} имеет вид 
\begin{gather*}
...
Докажем, что решение этого уравнение определено на интервале $[t_0, T]$. 
Заметим, что интеграл $\int_0^t \| A(\tau)\|  \ d\tau$ равномерно ограничен по $u(\cdot) $ на $  B_{\mathbb{L}_2}(0,\mu)$.
Для каждого фиксированного  $u(\cdot) $ найдется такое $\tau > t_0$, что решение \eqref{sec1:fundumental_matrix_eq} определено на интервале $ [t_0, \tau] $.
Выберем наибольшее из $\tau$.
Предположим, что $\tau < T$. 
Тогда проинтегрируем \eqref{sec1:fundumental_matrix_eq} от $t_0$ до $\tau$, получив
\begin{gather*}
...
Таким образом, можно заключить,  что при всех $u(\cdot) \in  B_{\mathbb{L}_2}(0,\mu)$, существует ограниченное решение  \eqref{sec1:fundumental_matrix_eq}, определенное на интервале $[t_0, T]$.
    
    
Напомним, что начальное условие $x_0 $~--- зафиксировано, а $ x(\cdot,u(\cdot))$ ~--- решение системы \eqref{sec1:common_nonlinear},  порожденное управлением $u(\cdot)$ и удовлетворяющее условию $x(t_0, u(\cdot)) = x_0$. 
...
\begin{lemma}\label{sec1:lem:frechet_derivative_common}
    Пусть выполнено  Предположение \ref{sec1:as:right_hand_side_conditions_global}.
    Тогда отображение $F$ имеет непрерывную производную Фреше, определяемую равенством $ F'(u(\cdot))\delta u(\cdot) =\delta x(T)$, где $\delta x(T)$ ~--- решение линеаризованной вдоль пары $\left( x(t,u(\cdot)),u(\cdot)\right)  $ системы \eqref{sec1:linearized_system}, порожденное управлением $\delta u(\cdot)$ при нулевом начальном условии.
\end{lemma}
\doc 
...
\begin{definition}
	Линейная система $\dot{y} = A(t) y + B(t) u $ называется управляемой на интервале времени $  t_0 \leqslant t \leqslant T $, если управляемой, если для любых $y_0, y_1 \in \mathbb{R}^n$ найдется управление $u(\cdot) \in \mathbb{L}_2$, переводящее систему из $y(0, u(\cdot)) = y_0 $ в $y(T, u(\cdot)) = y_1 $.
\end{definition}
...
Обозначим грамиан управляемости системы \eqref{sec1:linearized_system} через $W(T,u(\cdot))$, где $u(\cdot)$~--- управление, порождающее траекторию системы \eqref{sec1:common_nonlinear}, вдоль которой проводится линеаризация.
Тогда, из Определения \ref{sec1:def:grammian} и \eqref{sec1:lem2_assert} видно, что положительная определенность $W(T,u(\cdot))$ (управляемость системы \eqref{sec1:linearized_system} линеаризованной вдоль пары $\left( x(\cdot,u(\cdot)),u(\cdot)\right)   $) означает регулярность отображения $F$ в точке $u(\cdot) $. 
...
Пусть $X$, $Y$~--- гильбертовы пространства, а $f: X \rightarrow Y$~--- нелинейное отображение с производной Фреше $f'$, которая существует и непрерывна в некоторой области  $U \in X$.
    
Будем называть точку $a \in U$~--- регулярной точкой отображения $f$, если линейный оператор $f'(a)$ сюръективен: он отображает $X$ на $Y$, $ f'(a) X = Y $.
...
\begin{lemma}\label{sec1:lem:lip_fundumental_matrix}
    ...
    \end{gather*}
    где $X(t,s,u_1(\cdot)) $ и $X(t,s,u_2(\cdot)) $ --- фундаментальные матрицы систем \eqref{sec1:linearized_system}, линеаризованных вдоль траекторий $x(\cdot, u_1(\cdot)) $ и $x(\cdot, u_2(\cdot)) $ соответственно. 
\end{lemma}
\doc
...
Аналогично \eqref{sec1:fundumental_matrix_eq}, решение этого уравнение ограничено, то есть существует такая константа $k_X>0$ что
\begin{gather*}
...
Используя Лемму \ref{sec1:lem:lip_of_solutions_global} в условиях предположения \ref{sec1:as:right_hand_side_diff_lip}, мы можем получить оценку
...
\begin{theorem}\label{sec1:th:small_control_convexity}
    Пусть выполнены Предположения \ref{sec1:as:right_hand_side_conditions_global} и \ref{sec1:as:right_hand_side_diff_lip}, а также линеаризованная вдоль пары $\left( x(\cdot,0),0\right)   $ система \eqref{sec1:common_nonlinear} является управляемой.
...
где матрицы $X(\cdot, \cdot, 0)$  и $B(\cdot, 0)$ определены для системы
 \begin{gather}\label{sec1:linearized_system_zero_control}
 \begin{gathered}
 ...
 \end{gathered}
\end{gather}
 Система \eqref{sec1:linearized_system_zero_control} является линеаризацией системы \eqref{sec1:common_nonlinear} вдоль пары $\Big(x(\cdot, 0), 0\Big) $.
...
\end{gather*}
где $ \big(F'(0)\big)^* =  B^{\top} (\cdot, 0) X^{\top}(T, \cdot, 0)$.

Обозначим через  $W(T,0) $ грамиан управляемости системы \eqref{sec1:linearized_system_zero_control}.
Тогда 
\begin{gather}\label{sec1:self_adjoint_with_derivative}
    F'(0) \big(F'(0)\big)^* = \int\limits_{t_0}^{T} X(t, \tau, 0) B(\tau, 0) B^{\top} (\tau, 0) X^{\top}(T, \tau, 0) \ d\tau= W(T,0).
\end{gather}

Отсюда следует, что  регулярность отображения  $F$ в точке $u(\cdot) \equiv 0$ определяется положительной определенностью грамиана  $W(T,0) $, то есть управляемостью линеаризованной системы \eqref{sec1:linearized_system_zero_control}. 
...
Обозначим через $G_{\varepsilon}(\tau)$ множество достижимости системы \eqref{sec1:eps_nonlinear} в момент времени $\tau$
\begin{gather*}
...
\end{gather*}
По аналогии с \eqref{sec1:linearized_system_zero_control}, линеаризуем \eqref{sec1:eps_nonlinear} вдоль траектории $ z(\cdot,0) = x(\cdot, 0) $
\begin{gather}\label{sec1:eps_linearized}
    \delta\dot{z} = \widetilde{A}(\tau, 0) \delta z(\tau) +\widetilde{B}(\tau, 0) \delta \upsilon(t),\qquad 0 \leqslant \tau \leqslant 1,  \qquad \delta z(0) = 0,
\end{gather}
где $ \widetilde{A}(\tau, \upsilon(\cdot)) = \varepsilon A(\varepsilon \tau + t_0, u(\cdot)) $,  $\widetilde{B}(\tau, \upsilon(\cdot)) = B(\varepsilon \tau + t_0, u(\cdot)) $. 
Для сокращения записи введем обозначения $A_{\varepsilon}(\tau) = \widetilde{A}(\tau, 0)$ и $B_{\varepsilon}(\tau) = \widetilde{B}(\tau, 0)$.
Фундаментальная матрица $ X_{\varepsilon}(\tau,\xi) $ системы \eqref{sec1:eps_linearized} определяется, как решение уравнения
\begin{gather*}
    \frac{dX_{\varepsilon}(\tau,\xi)}{d\tau} = A_{\varepsilon}(\tau) X_{\varepsilon}(\tau,\xi), \qquad X_{\varepsilon}(\tau,\tau) = I. 
\end{gather*}

Если система \eqref{sec1:eps_nonlinear} была линеаризована вдоль траектории $ z(\tau, \upsilon(\cdot)) $, то фундаментальную матрицу такой линейной системы будем обозначать как $X(\cdot,\cdot, \upsilon(\cdot)) $.
Обозначим через $ W_{\varepsilon}(\tau, 0) $ грамиан управляемости системы \eqref{sec1:eps_linearized}. 
...
Фундаментальные матрицы систем \eqref{sec1:linearized_system} и \eqref{sec1:eps_linearized} эквивалентны с учётом замены времени и обозначения $ X_{\varepsilon}(\tau,\xi) = X(\varepsilon \tau + t_0,\varepsilon \xi + t_0, 0) $.
\begin{gather*}
...
\begin{gather*}
	S'(\upsilon(\cdot))\delta \upsilon(\cdot) = \int\limits_0^1  X_{\varepsilon}(1, \tau, \upsilon(\cdot)) \widetilde{B}(\tau, \upsilon(\cdot))  \delta\upsilon(\tau)\ d\tau. 
\end{gather*}
...
\begin{gather}\label{sec1:eps_lip_a}
 ...
 Применяя Лемму \ref{sec1:lem:lip_dx_global} к отображению $S$, получаем, что отображение $S'(\upsilon)$ удовлетворяет условию Липшица с константой 
 \begin{gather}\label{sec1:lip_eps}
 \begin{gathered}
...
Действительно, в этом случае, для любых $x_1, x_2 \in D$, $t\in [t_0, t_0 + \overline{\varepsilon}]$, выполняется $f_2(t,x_1) = f_2(t, x_2)$ и $\dfrac{\partial f_2}{\partial x} (t, x_1) = \dfrac{\partial f_2}{\partial x} (t, x_2)$, то соответствующие константы Липшица можно выбрать нулевыми:   $ L_{f_2} = 0$ и $l_{f_2} = 0$.
  
По аналогии с \eqref{sec1:self_adjoint_with_derivative}, для самосопряженного оператора $S'(0)\big(S'(0)\big)^*$ верно равенство 
\begin{gather*}
	S'(0)\big(S'(0)\big)^* = W_{\varepsilon}. 
\end{gather*}
  
Тогда, управляемость системы \eqref{sec1:eps_linearized} (минимальное собственное число $ \lambda(\varepsilon) $ грамиана управляемости $W_{\varepsilon}$ должно быть строго положительно) означает регулярность отображения $S$ при $\upsilon = 0$, а множество достижимости $G_{\varepsilon}$ есть образ гильбертова шара $B_{\mathbb{L}_2}(0,\mu\sqrt{\varepsilon})$ при его отображении $S$.
 
Используя теорему \ref{sec1:th:PolyakTh}, сформулируем следующую
\begin{theorem}\label{sec1:th:small_time_convexity}
     ...
     \begin{gather}\label{sec1:small_time_convexity_condition}
         ...
         \end{array}} \right.
     \end{gather}
  то существует такое $ \varepsilon_1 > 0 $, что множество достижимости $G(\varepsilon)$ системы \eqref{sec1:common_nonlinear_small_time} выпукло при всех $\varepsilon < \varepsilon_1 $.
 \end{theorem}
 \doc
Применим теорему \ref{sec1:th:small_control_convexity} к системе \eqref{sec1:eps_nonlinear}. 
Получаем, что множество достижимости $\widetilde{G}(\varepsilon)$ будет выпукло при выполнении  условия \eqref{sec1:mu0}, которое в этом случае примет вид
 \begin{gather}\label{sec1:th3_proof_origin_condition}
     \lambda(\varepsilon) \geqslant 4 \mu^2 \varepsilon L_u^2 (\varepsilon).
 \end{gather}
 Заметим, что $ \lambda(\varepsilon)  $ удовлетворяющие условию 
 \begin{gather}\label{sec1:th3_proof_condition_with_f2}
 ...
 \end{gather}
 будут удовлетворять и условию \eqref{sec1:th3_proof_origin_condition}. 
 Константы $L_0$ и $L_1$ определены в \eqref{sec1:lip_eps}.  
 В соответствии с Утверждением \ref{sec1:utv:connection_with_scaled_system} множества достижимости систем  \eqref{sec1:common_nonlinear_small_time} и \eqref{sec1:eps_nonlinear} совпадают, поэтому \eqref{sec1:th3_proof_condition_with_f2} является условие выпуклости не только множества достижимости $\widetilde{G}(\varepsilon)$ системы  \eqref{sec1:eps_nonlinear}, но и $G(\varepsilon)$ системы \eqref{sec1:common_nonlinear_small_time}.
 
 Рассмотрим случай, когда  $f_2(t,x)$ не зависит от $x$.
 Тогда  $L_0 = 0$ и условие \eqref{sec1:th3_proof_condition_with_f2} принимает вид
 \begin{gather}\label{sec1:th3_proof_condition_without_f2}
 	\lambda(\varepsilon) \geqslant   4 \mu^2  L_1^2 \varepsilon^3.
 \end{gather}
 
В этом случае нам необходимо доказать, что если для каких-то $K > 0$, $\alpha > 0$ и $ 0 < \varepsilon_0 < \overline{\varepsilon}$ условие $ \lambda(\varepsilon) \geqslant  K\varepsilon ^{3 - \alpha} $ выполняется при всех $\varepsilon \leqslant \varepsilon_0$, то существует такое $ \varepsilon_1 > 0 $, что при  $\varepsilon < \varepsilon_1 $ справедливо и условие \eqref{sec1:th3_proof_condition_without_f2}.
Для этого необходимо, чтобы 
\begin{gather}\label{sec1:th3_proof_final_condition_without_f2}
...
\end{gather}
 ...
 Обозначим $\varepsilon_1 = \min \left(\varepsilon_0, \sqrt[\alpha]{\frac{K}{4 \mu^2  L_1^2}}\right) $, тогда для всех $\varepsilon < \varepsilon_1 $ будет выполняться \eqref{sec1:th3_proof_final_condition_without_f2}, а значит и \eqref{sec1:th3_proof_condition_without_f2}.
 

Теперь вернемся к рассмотрению общего случая, когда $f_2(t,x)$ зависит от $x$.
Здесь мы доказываем, что если для каких-то $K > 0$, $\alpha > 0$ и $ 0 < \varepsilon_0 < \overline{\varepsilon}$ условие $ \lambda(\varepsilon) \geqslant  K\varepsilon ^{1 - \alpha} $ выполняется при всех $\varepsilon \leqslant \varepsilon_0$, то существует такое $ \varepsilon_1 > 0 $, что при  $\varepsilon < \varepsilon_1 $ справедливо и условие \eqref{sec1:th3_proof_condition_with_f2}. 
Для этого необходимо, чтобы 
\begin{gather}\label{sec1:th3_proof_final_condition_with_f2}
\begin{gathered}
...
\end{gather}
 ...
 Заметим, что правая часть обращается в нуль при $\varepsilon = 0$ и монотонно возрастает при $\varepsilon > 0$, а значит существует такое $\varepsilon_2 > 0$, что $ K = 4 \mu^2 \varepsilon_2^{\alpha} (L_0^2 + 2 L_0 L_1 \varepsilon_2 + L_1^2 \varepsilon_2^2)$. 
 Обозначим $\varepsilon_1 = \min \left(\varepsilon_0, \varepsilon_2\right) $, тогда для всех $\varepsilon < \varepsilon_1 $ будет выполняться \eqref{sec1:th3_proof_final_condition_with_f2}, а значит и \eqref{sec1:th3_proof_condition_with_f2}.
 
 \hfill$\square$\\[1ex]%--- P r o o f.
 
 \begin{zam}
 	Теорема \ref{sec1:th:small_time_convexity} дает удобное условие выпуклости множеств достижимости нелинейных систем \eqref{sec1:common_nonlinear_small_time} на малых интервалах времени, не требующее трудоемкого вычисления констант $L_0$ и $L_1$.
 	Однако, величина $\varepsilon_1$, полученная таким образом может оказаться консервативной. 
 	Для того, чтобы приблизить ее к границе, на которой условие \eqref{sec1:th3_proof_origin_condition} действительно перестанет выполняться, имеет смысл выбирать $\alpha$ как можно меньше, а $K$ как можно больше. 
 	Либо, если необходима точная оценка этой границы, использовать непосредственно условие \eqref{sec1:th3_proof_origin_condition}.
...
Для проверки условий теоремы \ref{sec1:th:small_time_convexity} необходимо оценивать асимптотику минимального собственного числа грамиана управляемости линеаризованной системы с малым параметром.
...
Требуется оценить зависимость минимального собственного числа $ \lambda_{\varepsilon} $ грамиана управляемости $ W_{\varepsilon} $ системы \eqref{sec1:eps_linear_system} от малого параметра $ \varepsilon  $.
\subsubsection{Рекуррентная процедура}
Напомним, что грамиан управляемости линейной стационарной вполне управляемой системы \eqref{sec1:eps_linear_system}~--- положительно-определенная симметричная матрица, определяемая равенством
\begin{gather*}
...
Если матрица $ A $ системы \eqref{sec1:eps_linear_system}~--- нильпотентна, со степенью нильпотентности $ k $, то ряд \eqref{sec1:gram1} конечен, так как коэффициенты $ U_i, \ i \geqslant k$ содержат $ A $ в степенях больше $ k$ и, следовательно, равны нулю.

В противном случае, необходимо убедиться, что ряд  \eqref{sec1:gram1}  сходится. 
...
где $C > 0$. 
Тогда 
\begin{gather}\label{sec1:compare}
		W_{\varepsilon} \leqslant \sum\limits_{i = 0}^{\infty} \frac{C \|A\|^i \|U_0\| \varepsilon^i}{(i + 1)!}.
\end{gather} 
 
 Отсюда следует, что ряд \eqref{sec1:gram1} сходится, так как сходится ряд справа в \eqref{sec1:compare}.
 
\subsubsection{Некоторые частные случаи}
Приведено  несколько примеров линейных систем второго порядка, для которых выполняется условие \eqref{sec1:small_time_convexity_condition}.
...
После этого, продемонстрируем применение описанного в предыдущем разделе метода к проверке выпуклости множеств достижимости двух нелинейных систем, используя теорему \ref{sec1:th:small_time_convexity}.
...
При малых $ \varepsilon $, существуют $\alpha >0 $ такие, что $ \varepsilon^2 >  \varepsilon^{3-\alpha}  $, следовательно,  для системы <<двойной интегратор>>, условие \eqref{sec1:small_time_convexity_condition} выполняется, а значит, множества достижимости систем, линеаризация которых вдоль отвечающей нулевому управлению траектории приводит к <<двойному интегратору>>, являются выпуклыми на малых промежутках времени.

\paragraph{Система второго порядка с одним входом.}
Можно показать, что любую линейную управляемую систему второго порядка можно привести к форме Фробениуса c помощью эквивалентных преобразований.
При этом, такие преобразования, не меняют асимптотику грамиана управляемости, так как не влияют на собственные числа матрицы $A$  системы.
...
Значит, из \eqref{sec1:lambdamax} следует, что при достаточно малых $\varepsilon$.
\begin{gather*}
...
\begin{gather}
    \lambda_{\varepsilon} \geqslant \lambda_{min}(S_{\varepsilon}^{2}) + \lambda_{min}(R(\varepsilon)) \geqslant  C \varepsilon^{3-\alpha}, \qquad C > 0, \quad \alpha > 0.
\end{gather}
...
Если матрица $ B $ имеет полный ранг, то $\lambda_{\varepsilon} \geqslant \beta $, где $ \beta $~--- некоторое положительное число, и условие \eqref{sec1:small_time_convexity_condition}, очевидно, выполняется.
...
Покажем, что ограничение на управление $ \upsilon $  можно представить в схожем с исходным ограничением на управление \eqref{sec1:contrainsts1} виде
\begin{gather*}
...
\end{gather}
Рассуждения выше доказывают, что из \eqref{sec1:contrainsts1} следует \eqref{sec1:constraints2}; обратное тоже верно, так как любое $ \upsilon(t) $, для которого справедливо $ \int_{0}^{1} \upsilon ^2 d \tau \leqslant (1 + c^2) $, можно представить в виде $ \upsilon = u_1 + c u_2 $, где 
\begin{gather*}
...
Видно, что малых значениях $\varepsilon$ множества достижимости выпуклы, но при увеличении $\varepsilon $ становятся невыпуклыми.
 % ссылка на тарасьева
...
Моделирование показывает, что множество достижимости <<машины Дубинса>> невыпукло и при достаточно малых $ \varepsilon $; это можно показать и теоретически, но строгое доказательство невыпуклости выходит за рамки этого раздела.
