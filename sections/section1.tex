...
Так как $A(t)$ измерима, то решение \eqref{sec1:fundumental_matrix_eq} определено в некоторой окрестности $t_0$ и единственно\cite[с. 46]{Fillipov2}. 
...
Условие \eqref{sec1:regularity_cond} называют условием Люстерника\cite{Dmitruk1980}.
...
\begin{theorem}\label{sec1:th:small_control_convexity}
    Пусть выполнены Предположения \ref{sec1:as:right_hand_side_conditions_global} и \ref{sec1:as:right_hand_side_diff_lip}, и пусть линеаризованная вдоль пары $\left( x(\cdot,0),0\right)   $ система \eqref{sec1:common_nonlinear} является управляемой.
...
По аналогии с \eqref{sec1:self_adjoint_with_derivative}, для самосопряженного оператора $S'(0)S'(0)^*$ верно равенство 
\begin{gather*}
...
\end{gather}
 ...
 Обозначим $\varepsilon_1 = \min \left(\varepsilon_0,  \left(\frac{K}{4 \mu^2  L_1^2}\right)^{\frac{1}{\alpha}} \right) $, тогда для всех $\varepsilon < \varepsilon_1 $ будет выполняться \eqref{sec1:th3_proof_final_condition_without_f2}, а значит и \eqref{sec1:th3_proof_condition_without_f2}.
...
\end{gather}
 ...
 Заметим, что правая часть \eqref{sec1:th3_proof_final_condition_with_f2} обращается в нуль при $\varepsilon = 0$ и непрерывна при $\varepsilon > 0$, а значит существует такое $\varepsilon_2 > 0$, что  \eqref{sec1:th3_proof_final_condition_with_f2} выполняется при всех $\varepsilon < \varepsilon_2 $.
...
Приведем  несколько примеров линейных систем второго порядка, для грамианов управляемости которых выполняется условие \eqref{sec1:small_time_convexity_condition}.
...
Рассмотрим невырожденное преобразование системы \eqref{sec1:eps_linear_system}
\begin{gather}\label{sec1:eps_transformed_system}
\begin{gathered}
...
По аналогии с \eqref{sec1:We} выпишем выражения для грамиана управляемости системы \eqref{sec1:eps_transformed_system} $\hat{W}_{\varepsilon}$  и коэффициентов разложения $\hat{U_i}$, $i \in \mathbb{N}$.
\begin{gather*}
...
Подставляя полученные выражения для $\hat{U}_i$ в \eqref{sec1:We}, получим, что  грамиан управляемости  системы \eqref{sec1:eps_transformed_system} связан с грамианом управляемости системы \eqref{sec1:eps_linear_system} равенством
\begin{gather}
...
Более того, так как $P$ --- невырожденная и квадратная матрица, которая не зависит от $\varepsilon$, то из \cite[теорема 4.5.10]{horn2012} вытекает, что $\lambda_{min}(W_{\varepsilon}) = \theta \lambda_{min}(\hat{W}_{\varepsilon}) $, причем $\theta $ зависит от $P$ и не зависит от $W_{\varepsilon} $ и $\hat{W}_{\varepsilon}$.
...
Моделирование показывает, что множество достижимости <<машины Дубинса>> невыпукло и при достаточно малых $ \varepsilon $.

Покажем, что множество достижимости системы  $G(\varepsilon)$ в данном случае также не является выпуклым при любом $\varepsilon >0$.
Рассмотрим задачу оптимального управления с линейным терминальным критерием
