...
В случае, где эта зависимость важна, будет использовать обозначения $A(\cdot, u(\cdot))$, $B(\cdot, u(\cdot))$, $X(\cdot, \cdot, u(\cdot))$. 
...
\end{gather}
\todo[inline]{О Б С У Д И Т Ь.  В этом доказательстве $\frac{\partial f_2}{\partial x}$ и $\beta$ --- это многомерные линейные операторы, действующие из $\mathbb{R}^n$ в $\mathbb{R}^{n \times r}$ и выражения вида $\beta(t) \Delta x(t)$ это не умножение, а применение этого оператора к вектору. }
\hfill $\square$
...
\begin{theorem}[\cite{Polyak2001, Polyak2001ru}]\label{s1:th:PolyakTh}
    ...
    {Будем называть множество $M $ сильно выпуклым, если существует такая константа $\rho > 0$, что для любой точки $x \in \partial M$ (границе множества), существует шар $B(y, \rho)$, такой, что $x \in B(y, \rho) \subset M$. То есть,  для каждой точки границы, существует шар радиуса $\rho$, который полностью содержится в $M$ и касается его границы в точке $x$.
...
\end{gather}
 ...
 \begin{zam}
 	Теорема \ref{s1:th:small_time_convexity} дает удобное условие достаточное выпуклости множеств достижимости нелинейных систем \eqref{s1:common_nonlinear_small_time} на малых интервалах времени, не требующее трудоемкого вычисления констант $L_0$ и $L_1$.
...
Таким образом, нелинейные системы второго порядка с интегральными ограничениями, линеаризация которых приводит к стационарным управляемым системам вида \eqref{s1:system32},  так же обладают выпуклыми множествами достижимости на малых промежутках времени.
Примером такой системы является осцилятор Дуффинга, подробно рассмотренный в разделе \ref{s2:examples}.
