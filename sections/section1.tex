...
\begin{theorem}\label{sec1:th:small_time_convexity}
     Множество достижимости $G(\varepsilon)$ системы \eqref{sec1:common_nonlinear_small_time} выпукло, если найдутся такие $K > 0$, $ \alpha \leqslant 0$, $ 0 < \varepsilon_0 < \bar{\varepsilon}$, что для всех $\varepsilon \leqslant \varepsilon_0$
 ...
 \end{gather}
 будут удовлетворять и условию \eqref{sec1:th3_proof_origin_condition}. Константы $L_0$ и $L_1$ определены в \eqref{sec1:lip_eps}.  
 В случае, если $f_2(t,x)$ не зависит от $x$, $L_0 = 0$ и условие \eqref{sec1:th3_proof_condition_with_f2} принимает вид
 \begin{gather}
 	\lambda(\varepsilon) \geqslant   4 \mu^2  L_1^2 \varepsilon^3.
 \end{gather}
 В этом случае, $ K =  4 \mu^2  L_1^2$, а $\alpha = 0$.
 В общем случае, когда $f_2(t,x)$ зависит от $x$, константы выбираются так, чтобы было выполнено условие
 \begin{gather*}
 	K \geqslant 4 \mu^2 L_0^2 \overline{\varepsilon}^{\alpha} + + 8 \mu^2 L_0 L_1 \overline{\varepsilon}^{1 + \alpha} +  4 \mu^2  L_1^2 \overline{\varepsilon}^{2 + \alpha}.
 \end{gather*}
 Такой выбор констант обеспечивает, что при выполнении условия \eqref{sec1:small_time_convexity_condition}, выполняется и условие \eqref{sec1:th3_proof_origin_condition}. 
 Справедливость этого неравенства, в соответствии с теоремой \ref{sec1:th:small_control_convexity} гарантирует выпуклость множеств достижимости системы \eqref{sec1:common_nonlinear_small_time}. 
 Но в соответствии с Утверждением \ref{sec1:utv:connection_with_scaled_system} множества достижимости систем  \eqref{sec1:common_nonlinear_small_time} и \eqref{sec1:eps_nonlinear} совпадают.
 \hfill$\square$\\[1ex]%--- P r o o f.
