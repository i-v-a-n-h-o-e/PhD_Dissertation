...
Следующий подраздел состоит иThe third section contains the formulation and proof of the main results. Finally, we provide two illustrative examples in the fourth section.
...
\begin{zam}
    Множество достижимости $G_{-}(T,\mu)$ системы \eqref{nonlinear_} совпадает с множеством нуль-\\-управляемости системы \eqref{sec22:nonlinear}, т.е. множетсва таких начальных условий, из которых система может быть переведена в начало координат управлениями из $B_{\mathbb{L}_2}(0,\mu) $ за время $T$. 
...
Это означает, что найдентся последовательность $ t_k \to T$ и $\delta > 0$ такие, что 
...
\end{gather*}
 ...
 Поскольку $t_k \to T$, $\rho(T-t_k) \to 0$, $\exp(\rho(T-t_k)) \to 1$, то существует такое $k_1$, что для всех $k > k_1$ выполено $\exp(\rho(T-t_k))\delta/2 \leqslant 2\delta/3$. 
Используя формулу
\begin{gather*}
...
мы получаем $ z^{\top}(t_k) W^{-1}(T-t_k) z(t_k) \leqslant 4\delta^2/9$.
 ...
 Это означает, что $V_{T}(t,z(t))  = z^{\top}(t)Q(t)z(t) \to 0$  as $t \to T$.
	\hfill $ \square $
