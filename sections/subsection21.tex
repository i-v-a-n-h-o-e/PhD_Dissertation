...
Эти условия оказываются аналогичны условиям, полученным в разделе \ref{s1:small_time_subsection}.
...
Напомним, что расстоянием Банаха-Мазура между выпуклыми компактными множествами $ X,Y \subset \mathbb R^n $, такими, что $0 \in \operatorname{int}\ X$, $0 \in \operatorname{int}\ Y$ называют величину $ \rho (X, Y)  $, определенную равенством 
\begin{gather*}
...
\begin{gather}\label{nonlinearY}
    \begin{gathered}
        \dot{x}(t)=f_1(t,x(t))+f_2(t,x(t))u(t), \qquad t_0 \leq t \leq t_0 + \overline{\varepsilon}, \qquad x(t_0) = x_0, \\
        y(t) = C x(t).
    \end{gathered}
\end{gather}
...
Как и в предыдущих разделах функции $ f_1: [t_0, t_0 + \varepsilon] \times \mathbb{R}^{n} \rightarrow \mathbb{R}^{n} $, $ f_2: [t_0, t_0 + \varepsilon] \times \mathbb{R}^{n} \rightarrow \mathbb{R}^{n \times r} $ предполагаются непрерывными по $(x, t)$ и обладающими непрерывными производными по $x$ на $ [t_0, t_0 + \varepsilon]$. 
...
\begin{definition}
    ...
    \begin{gather*}
        G_y(\varepsilon)=\{y\in \mathbb{R}^m:\exists u(\cdot)\in B_{\mathbb{L}_2}(0,\mu),\; y=Cx(t_0+\varepsilon, ,u(\cdot))\}.
    \end{gather*}
\end{definition}
...
Возможен случай $C = I$, в этом случае $y = x$ и $ G_y(\varepsilon) = G(\varepsilon)$


\begin{definition}
    Симметричная матрица, определённая равенством
    \begin{gather*}
        W_y(\varepsilon) = \int_{t_0}^{t_0+\varepsilon} C X(t_0+\varepsilon,t)B(t)B^{\top}(t)X^{\top}(t_0+\varepsilon,t) C^\top \, dt=CW(\varepsilon)C^\top,
    \end{gather*}
