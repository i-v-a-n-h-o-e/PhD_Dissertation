...
\geometry{a5paper,top=1.5cm,bottom=1.5cm,left=1.2cm,right=1.5cm}
\setlength{\topskip}{0pt}   %размер дополнительного верхнего поля
\setlength{\footskip}{12.3pt} % снимет warning, согласно https://tex.stackexchange.com/a/334346

\usepackage[main=russian,english, provide=*]{babel}   %% загружает пакет многоязыковой вёрстки
\addto\captionsrussian{\renewcommand{\refname}{Публикации автора по теме диссертации}}
...
\hypersetup{
    ...
    filecolor=magenta,      
    urlcolor=cyan,
    pdftitle={Osipov Thesis Abstract},
...
\ifdefined\showlabels
\fancyfoot[L]{\color{gray}\textbf{2026-01-14--14-40-01}} % Добавление текста в правый колонтитул
\fi
...
\fi
%\usepackage{refcheck}
% Подстрочная сноска с полной записью по ключу \bibitem
\newcommand{\footcite}[1]{%
	\footnote{\csname bibentry@#1\endcsname}%
}

\input{abstract_sections/biblio_copy}
...
\hyphenation{не-ли-ней-ных}

%-----------------------------------------------------------------------

\begin{document}
    \subfile{abstract_sections/title}
    \setcounter{page}{2} % Устанавливаем номер страницы после титульной
  	\subfile{abstract_sections/impressum}
    
    \subfile{abstract_sections/introduction}    
    \subfile{abstract_sections/summary}

    \subfile{common_sections/conclusion}
    
    \subfile{abstract_sections/biblio}
\end{document}